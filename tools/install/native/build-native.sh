...
ndk_archive=${FOLDGPT_NDK_ARCHIVE:-/mnt/c/Dev/FoldGPT/work/AndroidSdk-Linux/downloads/android-ndk-r29-linux.zip}
...
patch -p1 --fuzz=0 -d "$work/proot" < "$artifact/build/recipe/proot-shmat-errno.patch"
patch -p1 --fuzz=0 -d "$work/proot" < "$artifact/build/recipe/proot-kill-on-exit-sigterm.patch"
patch -p1 --fuzz=0 -d "$work/proot" < "$artifact/build/recipe/proot-strict-sandbox.patch"
