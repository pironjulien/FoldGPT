...
The coordinator, who owns ADB, can run from `C:\Dev\FoldGPT` after opening
the disposable IAB pane visibly and obtaining its current page target ID:

```powershell
python tools/gpu/check-client-raster.py --serial <device-serial> --target ACTUAL_DISPOSABLE_PAGE_TARGET --mode partial
```
...
```powershell
python tools/gpu/check-settings-menu.py --serial <device-serial> --target ACTUAL_MAIN_PAGE_TARGET
```
