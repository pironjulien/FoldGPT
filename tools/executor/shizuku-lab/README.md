...
```powershell
adb -s R3GL808JN4A install -r tools/executor/shizuku-lab/build/context-v1/app-debug.apk
adb -s R3GL808JN4A shell am start -W -n app.foldgpt.shizukuprobe/.ProbeActivity -a app.foldgpt.shizukuprobe.COLLECT
adb -s R3GL808JN4A shell run-as app.foldgpt.shizukuprobe cat files/report.json
```
...
```powershell
adb -s R3GL808JN4A shell am start -W -n app.foldgpt.shizukuprobe/.ProbeActivity -a app.foldgpt.shizukuprobe.RUN_FIXED
adb -s R3GL808JN4A shell run-as app.foldgpt.shizukuprobe cat files/report.json
```
