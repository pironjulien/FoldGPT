...
BASE = '/data/user/0/app.foldgpt/files/runas-native-v1'
PACKAGE = 'app.foldgpt.runasbrokerqualification.v1'
STAGE = SERVICE / 'build/runasbrokerqualification-v1/stage-r2'
...
def main():
    ...
    config = {'schema': 'foldgpt.runas-broker-config.v1', 'package': PACKAGE, 'target': 'app.foldgpt', 'base': BASE,
              'nativeLibraries': {name: sha(data) for name, data in sorted(native.items())},
              'runtimeFiles': len(runtime['dataFiles']), 'runtimeAliases': len(runtime['runtimeAliases'])}
    (assets / 'foldgpt-runas-broker-config.json').write_text(json.dumps(config, indent=2) + '\n')
    inventory = {'schema': 'foldgpt.runas-broker-build.v1', 'package': PACKAGE, 'base': BASE, 'androidExecuted': False,
