# Qualification run-as broker V1

Nouvel APK `app.foldgpt.runasbrokerqualification.v1`, nouvelle action
`RUN_BROKER_V1`, nouveau marker de l'Activity. Aucun ancien essai n'est rejoué.
...
`/data/user/0/app.foldgpt/files/runas-native-v1/python`, ainsi que tous les ELF
...
`runas-native-v1/workspace` et `runas-native-v1/broker` neufs. Les protections,
...
```powershell
python -B runasbrokerqualification/stage.py --python-cli <nouveau-cli> --python-cli-sha256 <sha-revu>
gradle :runasbrokerqualification:assembleDebug -PfoldgptFrozenTransportJni=C:/Dev/ChatgptFold/tools/executor/shizuku-lab/build/frozen-transport-jni --no-daemon --console=plain
```

CLI requis : RUNPATH exact
`/data/user/0/app.foldgpt/files/runas-native-v1/python/lib:$ORIGIN`.
Stage exclusif courant : `build/runasbrokerqualification-v1/stage-r2`.
...
Composant :
`app.foldgpt.runasbrokerqualification.v1/app.foldgpt.runasbrokerqualification.RunAsBrokerQualificationActivity`.
Actions : préfixe `app.foldgpt.runasbrokerqualification.v1.` suivi de
`COLLECT_INFO`, `AUTHORIZE`, `PREFLIGHT` ou `RUN_BROKER_V1`.
Rapport privé de l'APK : `files/runas-broker-v1/run_broker_v1.json`.
