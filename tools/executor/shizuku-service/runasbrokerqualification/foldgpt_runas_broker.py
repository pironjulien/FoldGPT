...
BASE = Path('/data/user/0/app.foldgpt/files/runas-native-v1')
WORKSPACE = BASE / 'workspace'
BROKER = BASE / 'broker'
SESSION = 'runas-broker-v1'
...
async def run(apk, expected_uid, expected_parent, nonce):
    result = {'schema': 'foldgpt.runas.broker-child.v1', 'nonce': nonce, 'pid': os.getpid(),
    ...
    try:
        ...
        challenge = ('foldgpt.runas.broker.v1:stdin:' + nonce + '\n').encode('ascii')
        received = bytearray()
        ...
        require(config['schema'] == 'foldgpt.runas-broker-config.v1' and config['base'] == str(BASE), 'Wrong installed config')
        ...
        emit(1, result)
        emit(2, {'schema': 'foldgpt.runas.broker-report.v1', 'nonce': nonce, 'pid': os.getpid(),
