...
#define ROOT "/data/user/0/app.foldgpt/files/runas-native-v1/python"

static const char *trial_nonce = "";
static int refuse(const char *stage) {
    int error = errno;
    fprintf(stderr, "{\"schema\":\"foldgpt.runas.broker-admission.v1\",\"stage\":\"%s\",\"errno\":%d,"
...
int main(int argc, char **argv) {
    ...
    alarm(30);
    if (argc != 7 || strcmp(argv[1], "--broker-v1") || strcmp(argv[3], "/data/user/0/app.foldgpt")
    ...
    *slash = 0; strcpy(directory, own);
    ...
    if (!directory_owned("/data/user/0/app.foldgpt", ur, 1)
        ...
        || !directory_owned("/data/user/0/app.foldgpt/files/runas-native-v1", ur, 1)
