...
public final class RunAsBrokerQualificationActivity extends Activity {
    private static final String PREFIX = "app.foldgpt.runasbrokerqualification.v1.";
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        if (!(PREFIX + "AUTHORIZE").equals(action) && !(PREFIX + "PREFLIGHT").equals(action)
                && !(PREFIX + "RUN_BROKER_V1").equals(action) && !(PREFIX + "COLLECT_INFO").equals(action)) {
        ...
        try {
            if ((PREFIX + "RUN_BROKER_V1").equals(action) && new File(directory(), "attempt-started-v1").exists()) {
            ...
            args = new Shizuku.UserServiceArgs(new ComponentName(this, RunAsBrokerQualificationService.class))
                .daemon(false).tag("foldgpt-runas-broker-v1").version(1)
                .processNameSuffix("runas_broker_v1").debuggable(false);
    ...
    private void bind() {
        ...
        try {
            ...
            if ((PREFIX + "RUN_BROKER_V1").equals(action)) {
    ...
    private File directory() throws Exception {
        File root = new File(getFilesDir().getCanonicalFile(), "runas-broker-v1");
    ...
    private void persist(JSONObject report) throws Exception {
        report.put("schema", "foldgpt.runas.broker-activity.v1").put("action", action)
