package app.foldgpt.runasqualification;
...
import app.foldgpt.shizukuexec.IRunAsQualification;
import app.foldgpt.shizukuexec.RunAsInstallation;
import app.foldgpt.shizukuexec.RunAsQualificationService;
...
/** Separate package, one identity trial; restored Intents cannot retry it. */
public final class RunAsQualificationActivity extends Activity {
    private static final String PREFIX = "app.foldgpt.runasqualification.v1.";
    ...
    private volatile IRunAsQualification service;
    ...
    private final ServiceConnection connection = new ServiceConnection() {
        @Override public void onServiceConnected(ComponentName name, IBinder binder) {
            ...
            service = IRunAsQualification.Stub.asInterface(binder);
            running = true;
            work.execute(RunAsQualificationActivity.this::run);
        }
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        if (!RunAsInstallation.QUALIFICATION.equals(getPackageName())) { fail("Wrong fixed APK identity"); return; }
        if (!(PREFIX + "AUTHORIZE").equals(action) && !(PREFIX + "PREFLIGHT").equals(action)
                && !(PREFIX + "RUN_IDENTITY_FDS_V1").equals(action) && !(PREFIX + "COLLECT_INFO").equals(action)) {
        ...
        if ((PREFIX + "COLLECT_INFO").equals(action)) {
            work.execute(() -> {
                try {
                    JSONObject result = RunAsInstallation.capture(this).put("state", "complete").put("nativeSpawnAttempted", false);
        ...
        try {
            if ((PREFIX + "RUN_IDENTITY_FDS_V1").equals(action) && new File(directory(), "attempt-started-v1").exists()) {
                text.setText("This identity trial was already reserved. The marker and report are retained; no retry.");
                finished = true; return;
            }
            text.setText("Preparing fixed run-as identity qualification");
            args = new Shizuku.UserServiceArgs(new ComponentName(this, RunAsQualificationService.class))
                .daemon(false).tag("foldgpt-runas-identity-v1").version(1)
                .processNameSuffix("runas_identity_v1").debuggable(false);
    ...
    private void bind() {
        ...
        try {
            ...
            if ((PREFIX + "RUN_IDENTITY_FDS_V1").equals(action)) {
                if (!new File(directory(), "attempt-started-v1").createNewFile()) {
    ...
    private void run() {
        ...
        try {
            JSONObject expected = RunAsInstallation.capture(this);
            JSONObject preflight = new JSONObject(service.preflight());
            RunAsInstallation.requireSame(expected, preflight);
    ...
    private void cancel() {
        IRunAsQualification current = service;
    ...
    private File directory() throws Exception {
        File root = new File(getFilesDir().getCanonicalFile(), "runas-identity-v1");
    ...
    private void persist(JSONObject report) throws Exception {
        report.put("schema", "foldgpt.runas.identity-activity.v1").put("action", action)
    ...
    @Override protected void onNewIntent(Intent intent) {
        ...
        setIntent(new Intent(this, RunAsQualificationActivity.class).setAction(intent.getAction())); recreate();
    }
