...
/** A fixed four-FD experiment, authenticated to its own diagnostic APK. */
public final class RunAsQualificationService extends IRunAsQualification.Stub implements IBinder.DeathRecipient {
    ...
    public RunAsQualificationService(Context context) {
    ...
    @Override public synchronized String preflight() {
        ...
        try {
            return RunAsInstallation.capture(context).toString();
    ...
    @Override public synchronized void start(IBinder requestedOwner, String expectedInstallation) {
        ...
        if (expectedInstallation == null || expectedInstallation.length() > 16384) throw new SecurityException("Installation record bound");
        try {
            JSONObject expected = new JSONObject(expectedInstallation);
            RunAsInstallation.requireSame(expected, RunAsInstallation.capture(context));
            owner = requestedOwner;
            owner.linkToDeath(this, 0);
            started = true;
            report = json("state", "starting");
            new Thread(() -> run(expected), "foldgpt-runas-identity-owner").start();
    ...
    private void run(JSONObject installation) {
        ...
        try {
            ...
            result.put("schema", "foldgpt.runas.identity-service.v1").put("installation", installation)
            ...
            synchronized (this) {
                ...
                String executable = directory + "/libfoldgpt_runas_identity.so";
                String[] argv = {"/system/bin/run-as", RunAsInstallation.TARGET, executable, "--identity-fds-v1",
            ...
            synchronized (this) {
                ...
                writeAndClose(input, "foldgpt.runas.identity.v1:stdin:" + nonce + "\n"); input = null;
                writeAndClose(control, "foldgpt.runas.identity.v1:control:" + nonce + "\n"); control = null;
            }
            ...
            RunAsInstallation.requireSame(installation, RunAsInstallation.capture(context));
            result.put("installationUnchanged", true);
            boolean passed = waitStatus == 0 && child.getBoolean("passed") && receipt.getBoolean("passed")
                && "foldgpt.runas.identity-child.v1".equals(child.getString("schema"))
                && "foldgpt.runas.identity-report.v1".equals(receipt.getString("schema"))
            ...
            try {
                boolean clean = (pid < 0 || reaped) && pipesClosed
