"""Verify and preserve the actual V11 APK on Windows. No device operations."""
import argparse
import hashlib
import json
import os
...
REPO = HERE.parents[2]
PACKAGE = "app.foldgpt.kernelqualification.v11"
BASE = "/data/local/tmp/foldgpt-bionic-supervisor-qualification-v3"
...
def main():
    ...
    stage = HERE / "build/qualification-stage-v11"
    source_apk = HERE / "qualification/build/outputs/apk/debug/qualification-debug.apk"
    ...
    with zipfile.ZipFile(apk) as archive:
        ...
        assert deployment["backendFactory"] == "tools.executor.bionic-supervisor.qualification_factory_v11:factory"
        ...
        assert all(value.encode() + b"\0" in dex for value in
                   ("kernel-v11", "foldgpt-kernel-qualification-v11", ".KERNEL_RUN_FIXED_V11", ".KERNEL_AUTHORIZE"))
    ...
    assert "android:versionCode(0x0101021b)=(type 0x10)0xb" in binary
    ...
    tests = []
    for path in sorted((HERE / "build/modules/transport/test-results/testDebugUnitTest").glob("TEST-*.xml")):
    ...
    sources = {HERE / name for name in ("build.gradle", "settings.gradle", "stage-qualification.py",
               "verify-qualification-v11.py", "qualification-inputs-v11.json", "transport/build.gradle", "qualification/build.gradle")}
    ...
    report = {"schema": "foldgpt.independent-qualification-apk.v1", "verified": True,
              "androidExecution": False, "package": PACKAGE, "versionCode": 11, "base": BASE,
