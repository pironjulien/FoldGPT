...
public final class RunAsBrokerQualificationService extends IRunAsBrokerQualification.Stub implements IBinder.DeathRecipient {
    ...
    private void run(JSONObject installation) {
        ...
        try {
            ...
            result.put("schema", "foldgpt.runas.broker-service.v2").put("installation", installation)
            ...
            synchronized (this) {
                ...
                String[] argv = {"/system/bin/run-as", RunAsBrokerInstallation.TARGET, executable, "--broker-v2",
            ...
            synchronized (this) {
                ...
                writeAndClose(input, "foldgpt.runas.broker.v2:stdin:" + nonce + "\n"); input = null;
            ...
            nativeCleanup = finalReceipt.optInt("pid") == pid && nonce.equals(finalReceipt.optString("nonce"))
                && (("foldgpt.runas.broker-report.v2".equals(finalReceipt.optString("schema"))
                    && finalReceipt.optBoolean("cleanupComplete"))
                    || ("foldgpt.runas.broker-admission.v2".equals(finalReceipt.optString("schema"))
            ...
            boolean passed = waitStatus == 0 && child.getBoolean("passed") && receipt.getBoolean("passed")
                && "foldgpt.runas.broker-child.v2".equals(child.getString("schema"))
                && "foldgpt.runas.broker-report.v2".equals(receipt.getString("schema"))
