...
public final class RunAsBrokerInstallation {
    ...
    public static final String QUALIFICATION = "app.foldgpt.runasbrokerqualification.v2";
    ...
    public static JSONObject capture(Context context) throws Exception {
        ...
        if (!"foldgpt.runas-broker-config.v2".equals(config.getString("schema"))
                || !QUALIFICATION.equals(config.getString("package"))) throw new SecurityException("Wrong broker config");
        if (target.getInt("uid") != config.getInt("qualificationUid")
        ...
        return new JSONObject().put("schema", "foldgpt.runas.installation.v2").put("target", target)
