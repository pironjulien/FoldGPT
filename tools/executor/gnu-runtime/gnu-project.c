...
/* Fixed GNU Python project diagnostic, derived from probe-landlock-codex.c.
...
#include "gnu-project.generated.h"
...
static void grant_path(int ruleset, const char *path, uint64_t rights, int optional) {
...
static void child_probe(int channel, int workspace, int base, int outside, pid_t owner) {
    ...
    worker_check("outside_read_denied", mode_fd < 0 && errno == EACCES, errno);
    ...
    execl(proot_path, proot_path,
          "--kill-on-exit",
          "--strict-sandbox", "-r", rootfs_path, "-w", "/foldgpt-fixture", "-b", "/dev", "-b", "/proc",
#ifdef __ANDROID__
          "-b", "/system", "-b", "/apex",
#endif
          ...
          "/usr/bin/python3", "-I", "-B", "-c", gnu_project_script, (char *)NULL);
...
int main(int argc, char **argv) {
...
#ifdef __ANDROID__
    int length = snprintf(evidence, sizeof(evidence), "%s/cache/foldgpt-gnu-project-XXXXXX", parent_path);
    path_join(proot_path, argv[2], "libfoldgpt-strict-proot.so");
...
#else
    int length = snprintf(evidence, sizeof(evidence), "%s/foldgpt-gnu-project-XXXXXX", parent_path);
    ...
    verify_file(scratch_directory, "project/dist/result.txt", "native GNU project: 42\n");
    ...
    require(scratch_grants > 0 && scratch_mode_changes > 0,
    ...
    puts("scope=real Bash/GNU Python project under default-deny Landlock and seccomp; no Desktop or managed-policy routing");
    return 0;
}
