...
static __attribute__((unused)) int mg_static_reference_elf(int fd) {
    Elf64_Ehdr header;
...
static void mg_launch(struct config *c,int input,int output,int error,int setup,int channel,pid_t supervisor) {
    ...
    if(setpgid(0,0)<0||prctl(PR_SET_PDEATHSIG,SIGKILL,0,0,0)<0||getppid()!=supervisor||
       dup2(input,0)<0||dup2(output,1)<0||dup2(error,2)<0||gnu_f2fs_probe()<0||ptrace(PTRACE_TRACEME,0,NULL,NULL)<0||raise(SIGSTOP)!=0)goto failed;
    failure.stage=2;
    ...
    failure.stage=4;
    int listener=gnu_notification_filter();
    if(listener<0||mg_send_listener(channel,listener)<0)goto failed;
    close(listener);close(channel);
    /* The opt-in GNU filter above is complete; the static filter is a different profile. */
    failure.stage=5;
...
static int mg_decide(const struct seccomp_notif *request,uint64_t address,const char *path,uint64_t flags,uint64_t mode,struct mg_decision *d) {
    ...
    int length=snprintf(frame,sizeof(frame),"{\"type\":\"%s\",\"id\":%"PRIu64",\"syscall\":%d,\"flags\":%"PRIu64",\"mode\":%"PRIu64",\"pathHex\":\"%s\",\"pid\":%u,\"pathAddress\":%"PRIu64",\"howAddress\":%"PRIu64"}",gnu_policy_tmp?"openTmp":"open",id,nr,flags,mode,hexpath,request->pid,address,how_address);
...
static int mg_open_file(int root_fd,const char *relative,uint64_t flags,uint64_t mode,const struct mg_decision *d) {
    ...
    int parent=(int)syscall(SYS_openat2,root_fd,parent_path,&how,sizeof(how));
...
static __attribute__((unused)) int mg_workspace_notification(int listener,const struct seccomp_notif *request) {
    ...
    int file=mg_open_file(mg_root,path,flags,mode,&decision);
...
#include "gnu-managed-operations.h"
...
static int mg_supervise(struct config *c) {
    ...
    while(alive||!eof[0]||!eof[1]||!eof[2]) {
        ...
        if(!terminating&&(polls[3].revents&POLLIN)) {
            ...
            else if(gnu_notification(listener,request)<0)goto runtime_failed;
        }
...
int main(int argc,char **argv) {
    ...
    if(mg_exec<0||fstat(mg_exec,&executable)<0||!S_ISREG(executable.st_mode)||(executable.st_mode&07000)||
       !ordinary_filesystem(mg_exec)||gnu_dynamic_elf(mg_exec)<0)return fail("managed-static-executable");
