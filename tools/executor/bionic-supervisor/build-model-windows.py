"""Freeze and build the managed model supervisor with Windows NDK r29.

No WSL, Android execution, dependency installation or
...
def main():
    ...
    frozen.mkdir()
    inputs = {
        "runner.c": ROOT / "tools/executor/bionic-supervisor/runner.c",
        ...
        "build-model-windows.py": Path(__file__).resolve(),
    }
    ...
    executable = output / "libfoldgpt_bionic_supervisor.so"
    command = [str(compiler), "--target=aarch64-linux-android35", "-std=c11", "-O2",
        "-Wall", "-Wextra", "-Werror", "-fPIE", "-pie", "-fstack-protector-strong",
        "-I" + str(frozen), "-MD", "-MF", str(output / "headers.d"), "-MT", "model-runner.o",
        str(frozen / "runner.c"), "-Wl,--no-undefined,-z,relro,-z,now,-z,noexecstack",
    ...
    spec = importlib.util.spec_from_file_location("foldgpt_model_elf_check", frozen / "check-elf.py")
    ...
    report = {"schema": "foldgpt.native-model-build.v1", "androidExecuted": False,
        "wslExecuted": False, "ndk": "29.0.14206865", "apiLevel": 35,
        "compilerSha256": digest(compiler), "readelfSha256": digest(readelf),
        "sourceSha256": digest(frozen / "runner.c"),
        "profileHeaderSha256": digest(frozen / "native-runner-seccomp.h"),
        "includedHeaders": len(headers), "reproducible": True,
        "elf": evidence, "modelRunnerSourceChanged": digest(frozen / "runner.c") !=
            "7d1b2b66763ec16d5ba49a25a001ca02899ab9b67d3c72a4a3cc23524b279805"}
