#ifndef FOLDGPT_HOST_FD_SECCOMP_H
#define FOLDGPT_HOST_FD_SECCOMP_H

/* Separate human profile derived from native-runner-seccomp.h SHA256
 * ea8bc58f260f08911724b45c941e103a606e6c8d787df8aa458ad91bc18dd657.
 * FIOCLEX/FIONCLEX change only this process's descriptor inheritance flag,
 * exactly as already-admitted fcntl(F_SETFD). No terminal/device control or
 * pointer input is admitted. CPython fopen uses FIOCLEX before returning.
 * The qualified model header and runner remain unchanged. */
#include <sys/ioctl.h>
...
#endif
#ifdef __NR_fcntl
        /* Two integer-only descriptor commands; every other ioctl is denied.
