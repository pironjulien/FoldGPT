#ifndef FOLDGPT_HOST_PTY_SECCOMP_H
#define FOLDGPT_HOST_PTY_SECCOMP_H

/* Separate PTY profile derived from native-runner-seccomp.h SHA256
 * ea8bc58f260f08911724b45c941e103a606e6c8d787df8aa458ad91bc18dd657.
 * The worker already owns one private controlling terminal in a fresh session.
 * Only that terminal device may be inherited/acquired; its master never enters
 * the worker. No model process uses this header. */
#include <sys/ioctl.h>
#include <termios.h>
...
#endif
#ifdef __NR_fcntl
        /* Terminal controls are confined to the private slave capability.
         * Kernel tty/session checks govern foreground groups. TIOCSTI,
         * TIOCLINUX, TIOCCONS, TIOCSCTTY/TIOCNOTTY and arbitrary device ioctls
         * are intentionally absent. Upper request bits must be zero.
         * 16 operations => high check(3), low load(1), pairs(32), deny(1). */
        BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_ioctl, 0, 37),
        BPF_STMT(BPF_LD | BPF_W | BPF_ABS, NR_SC_ARG_HI(1)),
        BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, 0, 1, 0),
        BPF_STMT(BPF_RET | BPF_K, NR_SC_DENY),
        BPF_STMT(BPF_LD | BPF_W | BPF_ABS, NR_SC_ARG_LO(1)),
        NR_SC_ALLOW_NR(TCGETS),
        NR_SC_ALLOW_NR(TCSETS),
        NR_SC_ALLOW_NR(TCSETSW),
        NR_SC_ALLOW_NR(TCSETSF),
        NR_SC_ALLOW_NR(TIOCGWINSZ),
        NR_SC_ALLOW_NR(TIOCSWINSZ),
        NR_SC_ALLOW_NR(TIOCGPGRP),
        NR_SC_ALLOW_NR(TIOCSPGRP),
        NR_SC_ALLOW_NR(TIOCGSID),
        NR_SC_ALLOW_NR(FIONREAD),
        NR_SC_ALLOW_NR(TIOCOUTQ),
        NR_SC_ALLOW_NR(TCXONC),
        NR_SC_ALLOW_NR(TCFLSH),
        NR_SC_ALLOW_NR(TCSBRK),
        NR_SC_ALLOW_NR(FIOCLEX),
        NR_SC_ALLOW_NR(FIONCLEX),
        BPF_STMT(BPF_RET | BPF_K, NR_SC_DENY),
        /* Fresh controlling session isolates all new groups. setsid stays
         * denied, and supervisor cleanup follows children through pidfds. */
        NR_SC_ALLOW(setpgid),
