...
import importlib
_base = importlib.import_module("tools.executor.bionic-supervisor.wire")
frame, seal = _base.frame, _base.seal

MAGIC = b"FGBT0001"
HEADER = struct.Struct("<8s8Q4I")


def envelope(*, cwd, executable, argv, environment, rows, cols, wall_ms=None, data_bytes=None,
    ...
    if any(type(value) is not int or not 1 <= value <= 65535 for value in (rows, cols)):
        raise ValueError("Explicit nonzero native terminal rows and columns required")
    payload = HEADER.pack(MAGIC, *(value or 0 for value in values), len(argv), len(environment), rows, cols)
