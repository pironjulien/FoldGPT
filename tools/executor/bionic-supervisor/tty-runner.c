...
#define PROFILE "bionic-direct-pty-v1"
struct configuration {
    uint64_t wall,data,file,output,tasks,cpu,descriptors,grace;
    uint32_t argc,envc,rows,cols;
...
static int terminal_master=-1;
struct input_stream {
    unsigned char data[65536];
    size_t begin,end;
    int eof;
};
static struct input_stream input;
...
static int configuration(int fd) {
    ...
    if(fstat(fd,&st)<0||st.st_size<88||st.st_size>MAX_FRAME||seals<0||(seals&required)!=required){errno=EPROTO;return -1;}
    ...
    if(memcmp(cursor,"FGBT0001",8)){free(data);errno=EPROTO;return -1;}cursor+=8;
    ...
    uint32_t dimensions[2];memcpy(dimensions,cursor,sizeof(dimensions));cursor+=sizeof(dimensions);
    cfg.rows=dimensions[0];cfg.cols=dimensions[1];
    if(!cfg.rows||cfg.rows>65535||!cfg.cols||cfg.cols>65535){free(data);errno=EINVAL;return -1;}
...
static int commands(void) {
    for(unsigned i=0;i<32;i++){
        unsigned char data[13];struct iovec vec={data,sizeof(data)};
        struct msghdr message={.msg_iov=&vec,.msg_iovlen=1};
        ssize_t size=recvmsg(cfg.command,&message,MSG_DONTWAIT);
        if(size==0){cancellation=1;return 0;}
        if(size<0){if(errno==EINTR)continue;if(errno==EAGAIN||errno==EWOULDBLOCK)return 0;return -1;}
        if(message.msg_flags&(MSG_TRUNC|MSG_CTRUNC)){errno=EPROTO;return -1;}
        if(size==1&&data[0]=='T'){cancellation=1;continue;}
        if(size==9&&data[0]=='I'){
            uint64_t request;memcpy(&request,data+1,8);
            if(!request){errno=EPROTO;return -1;}
            int32_t error=0;
            if(leader_pidfd<0||leader_reaped)error=ESRCH;
            else {
                siginfo_t info={0};
                if(waitid(P_PIDFD,(id_t)leader_pidfd,&info,WEXITED|WNOHANG|WNOWAIT|__WALL)<0)error=errno;
                else if(!info.si_pid){
                    /* The leader's setsid makes its pinned PID the initial
                     * PGID. This is the model interrupt contract; terminal
                     * keyboard VINTR targets the foreground job separately. */
                    if(syscall(SYS_pidfd_send_signal,leader_pidfd,SIGINT,NULL,PIDFD_SIGNAL_PROCESS_GROUP)<0)error=errno;
                    else signal_count++;
                }
            }
            unsigned char reply[13];reply[0]='i';memcpy(reply+1,&request,8);memcpy(reply+9,&error,4);
            ssize_t sent=send(cfg.command,reply,sizeof(reply),MSG_DONTWAIT|MSG_NOSIGNAL);
            if(sent!=sizeof(reply)){if(sent>=0)errno=EIO;return -1;}
            continue;
        }
        if(size==13&&data[0]=='R'){
            uint64_t request;uint16_t rows,cols;
            memcpy(&request,data+1,8);memcpy(&rows,data+9,2);memcpy(&cols,data+11,2);
            if(!request||!rows||!cols){errno=EPROTO;return -1;}
            int error=0;
            struct winsize intended={.ws_row=rows,.ws_col=cols},actual={0};
            if(terminal_master<0||leader_reaped)error=ESRCH;
            else if(ioctl(terminal_master,TIOCSWINSZ,&intended)<0||
                    ioctl(terminal_master,TIOCGWINSZ,&actual)<0)error=errno;
            else if(actual.ws_row!=rows||actual.ws_col!=cols)error=EIO;
            unsigned char reply[17];reply[0]='r';memcpy(reply+1,&request,8);
            memcpy(reply+9,&actual.ws_row,2);memcpy(reply+11,&actual.ws_col,2);
            int32_t code=error;memcpy(reply+13,&code,4);
            ssize_t sent=send(cfg.command,reply,sizeof(reply),MSG_DONTWAIT|MSG_NOSIGNAL);
            if(sent!=sizeof(reply)){if(sent>=0)errno=EIO;return -1;}
            continue;
        }
...
static void child(int output,int error,int setup,int gate,pid_t parent,uint64_t tasks) {
    ...
    if(prctl(PR_SET_PDEATHSIG,SIGKILL,0,0,0)<0||getppid()!=parent||identity()<0||
       setsid()<0||ioctl(output,TIOCSCTTY,0)<0||
       dup2(output,0)<0||dup2(output,1)<0||dup2(error,2)<0)goto failed;
    stage=2;
...
static int pump(struct output_stream *stream,int cancelling) {
    ...
    if(!stream->eof&&stream->end<sizeof(stream->data)){
        ...
        else if(!count||(count<0&&stream->fd==terminal_master&&errno==EIO))stream->eof=1;
        else if(errno!=EINTR&&errno!=EAGAIN&&errno!=EWOULDBLOCK)return -1;
    }
    return 0;
}
static int pump_input(int cancelling) {
    /* Closing the incoming pipe is not a PTY half-close. Never inject VEOF,
     * alter terminal mode or close the master while output/children remain. */
    if(cancelling){
        if(!input.eof){close(cfg.input);input.eof=1;}
        input.begin=input.end=0;return 0;
    }
    if(input.begin<input.end){
        ssize_t count=write(terminal_master,input.data+input.begin,input.end-input.begin);
        if(count>0)input.begin+=(size_t)count;
    ...
    if(!input.eof&&input.end<sizeof(input.data)){
        ssize_t count=read(cfg.input,input.data+input.end,sizeof(input.data)-input.end);
        if(count>0)input.end+=(size_t)count;
        else if(!count){close(cfg.input);input.eof=1;}
        else if(errno!=EINTR&&errno!=EAGAIN&&errno!=EWOULDBLOCK)return -1;
    }
    return 0;
}
...
static int supervise(void) {
    int out[2],setup[2],gate[2];uint64_t tasks;
    struct winsize initial={.ws_row=(unsigned short)cfg.rows,.ws_col=(unsigned short)cfg.cols};
    if(openpty(&out[0],&out[1],NULL,NULL,&initial)<0)return no_worker_result(10,errno);
    terminal_master=out[0];
    if(fcntl(out[0],F_SETFD,FD_CLOEXEC)<0||fcntl(out[1],F_SETFD,FD_CLOEXEC)<0||
       pipe2(setup,O_CLOEXEC)<0||pipe2(gate,O_CLOEXEC)<0||
       nonblock(cfg.input)<0||uid_task_ceiling(&tasks)<0)return no_worker_result(10,errno);
    pid_t parent=getpid();leader=fork();
    if(!leader)child(out[1],out[1],setup[1],gate[0],parent,tasks);
    int fork_error=errno;
    close(out[1]);close(setup[1]);close(gate[0]);
    ...
    close(gate[1]);
    /* The PTY merges child stdout/stderr. Owner stderr is an actual empty
     * pipe; its EOF is still independently observed by the Python owner. */
    struct output_stream streams[2]={{.fd=out[0],.destination=1},{.fd=-1,.destination=2,.eof=1}};
    if(nonblock(out[0])<0||nonblock(setup[0])<0){if(!launch_error)launch_error=errno;}
    ...
    for(;;){
        ...
        if(pump_input(terminating)<0){outcome="broker_error";cancellation=1;}
        ...
        struct pollfd wait[8]={{!streams[0].eof&&streams[0].end<sizeof(streams[0].data)?streams[0].fd:-1,POLLIN,0},
            ...
            {!input.eof&&input.end<sizeof(input.data)?cfg.input:-1,POLLIN,0},
            {input.begin<input.end?terminal_master:-1,POLLOUT,0}};
        (void)poll(wait,8,20);
    }
    ...
    close(out[0]);terminal_master=-1;close(setup[0]);
    if(!input.eof){close(cfg.input);input.eof=1;}
