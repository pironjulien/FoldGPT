/* Development fork of runner.c SHA256 7d1b2b66763ec16d5ba49a25a001ca02899ab9b67d3c72a4a3cc23524b279805. Model source remains frozen. */
...
static int configuration(int fd) {
  ...
  if(memcmp(p,"FGBH0001",8)){errno=EPROTO;return -1;}p+=8;
  ...
  while(!empty||!out_eof||!err_eof||!setup_eof){
    ...
    for(int i=0;i<3;i++)if(p[i].revents&(POLLIN|POLLHUP|POLLERR)){
      ...
      if(!n){if(i==0)out_eof=1;else if(i==1)err_eof=1;else setup_eof=1;}
        ...
        if(cfg.output&&total+amount>cfg.output){amount=(size_t)(cfg.output-total);cancelled=1;outcome="output_limit";}
    ...
    if(setup_eof&&!started&&!terminating&&!cancelled&&milliseconds()<deadline){
      ...
      else{const char *event="{\"type\":\"started\",\"profile\":\"bionic-host-v1\",\"setupCompleted\":true}";
...
int main(int argc,char **argv) {
  ...
  const char *ready="{\"type\":\"ready\",\"profile\":\"bionic-host-v1\"}";
