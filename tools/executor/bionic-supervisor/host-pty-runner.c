/* Separate PTY fork of host-runner.c SHA256 74d59953522d0880739b259d161debb21045271a9bf7876598569f0c00cbe493. Frozen pipe/model sources are unchanged. */
/* SPDX-License-Identifier: GPL-3.0-only
 * Separate human native process supervisor. Every protected pathname syscall
 * is mediated; no pointer-bearing syscall is approved with CONTINUE.
 */
#define _GNU_SOURCE
#include "host-pty-seccomp.h"
#include <dirent.h>
...
static int commands(void) {
  for(int i=0;i<32;i++) {
    unsigned char b[32];struct iovec v={b,sizeof(b)};struct msghdr m={.msg_iov=&v,.msg_iovlen=1};
    ssize_t n=recvmsg(cfg.command,&m,MSG_DONTWAIT|MSG_CMSG_CLOEXEC);
    if(!n){cancelled=1;return 0;}
    if(n<0){if(errno==EAGAIN||errno==EWOULDBLOCK)return 0;if(errno==EINTR)continue;return -1;}
    if(m.msg_flags&(MSG_TRUNC|MSG_CTRUNC)){errno=EPROTO;return -1;}
    if(n==13&&b[0]=='R'){
      uint64_t request;uint16_t rows,columns;memcpy(&request,b+1,8);memcpy(&rows,b+9,2);memcpy(&columns,b+11,2);
      int error=0;struct winsize size={.ws_row=rows,.ws_col=columns},actual={0};
      if(!request||!rows||!columns)error=EINVAL;
      else if(cfg.pty_master<0||reaped||cancelled)error=ESRCH;
      else if(ioctl(cfg.pty_master,TIOCSWINSZ,&size)<0||ioctl(cfg.pty_master,TIOCGWINSZ,&actual)<0)error=errno;
      else if(actual.ws_row!=rows||actual.ws_col!=columns)error=EIO;
      unsigned char reply[17];reply[0]='r';memcpy(reply+1,&request,8);memcpy(reply+9,&rows,2);memcpy(reply+11,&columns,2);memcpy(reply+13,&error,4);
      if(send(cfg.command,reply,sizeof(reply),MSG_NOSIGNAL|MSG_DONTWAIT)!=(ssize_t)sizeof(reply))return -1;
      continue;
    }
    if(n!=1||(b[0]!='I'&&b[0]!='T')){errno=EPROTO;return -1;}
    if(b[0]=='T')cancelled=1;
    else if(leader>0&&!reaped&&ioctl(cfg.pty_master,TIOCSIG,SIGINT)<0&&errno!=ESRCH)return -1;
  }
  return 0;
}
...
static int configuration(int fd) {
  ...
  if(memcmp(p,"FGBT0001",8)){errno=EPROTO;return -1;}p+=8;
  ...
  if(!cfg.argc||cfg.argc>256||cfg.envc>128||cfg.grants>64||cfg.wall>9007199254740991ULL||
     cfg.data<16777216||cfg.data>2147483648ULL||!cfg.file||cfg.file>1073741824||
     cfg.output>9007199254740991ULL||!cfg.tasks||cfg.tasks>128||cfg.cpu>9007199254740991ULL||
  ...
  if(end-p!=4){errno=EPROTO;return -1;}
  memcpy(&cfg.rows,p,2);memcpy(&cfg.columns,p+2,2);
  if(!cfg.rows||!cfg.columns){errno=EINVAL;return -1;}return 0;
}
...
static int setup_paths(void) {
  ...
  if(!strcmp(cfg.cwd,"/"))cfg.cwd_fd=open("/",O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW);
    ...
    const char *relative=cfg.cwd+length;
    if(*relative=='/')relative++;
    cfg.cwd_fd=beneath(cfg.root,*relative?relative:".",O_RDONLY|O_DIRECTORY,0);
  }
  if(cfg.cwd_fd<0)return -1;
  for(unsigned i=0;i<cfg.grants;i++){
...
static int landlock(void) {
  ...
  if(!result)result=ll_rule(rs,cfg.pty_pin,READ_FILE|WRITE_FILE|(1ULL<<15));
...
static void child(int terminal,int setup,int listener_socket,pid_t parent,rlim_t tasks) {
  int stage=1;
  if(setsid()<0||ioctl(terminal,TIOCSCTTY,0)<0||tcsetpgrp(terminal,getpgrp())<0||
     prctl(PR_SET_PDEATHSIG,SIGKILL,0,0,0)<0||getppid()!=parent||
     dup2(terminal,0)<0||dup2(terminal,1)<0||dup2(terminal,2)<0||identity()<0||
     prctl(PR_SET_NO_NEW_PRIVS,1,0,0,0)<0)goto failed;
  stage=2;
  if(fchdir(cfg.cwd_fd)<0||limit(RLIMIT_CORE,0)<0||limit(RLIMIT_DATA,cfg.data)<0||
     limit(RLIMIT_FSIZE,cfg.file)<0||limit(RLIMIT_NOFILE,cfg.descriptors)<0||(cfg.cpu&&limit(RLIMIT_CPU,cfg.cpu)<0)||limit(RLIMIT_NPROC,tasks)<0)goto failed;
...
static int own_terminal(int listener,const struct seccomp_notif *n,int mem,int kind,uint64_t flags,uint64_t output) {
  int result=-1,error=0;
  if(kind==0){
    uint64_t allowed=O_ACCMODE|O_CLOEXEC|O_NOCTTY|O_NONBLOCK|O_LARGEFILE;
    if(flags&~allowed||(flags&O_ACCMODE)==O_ACCMODE)error=EACCES;
    else{
      char proc[80];snprintf(proc,sizeof(proc),"/proc/self/fd/%d",cfg.pty_pin);
      int fd=open(proc,(int)flags|O_NOCTTY|O_CLOEXEC);
      if(fd<0)error=errno;
      else{
        struct stat pin,actual;
        if(fstat(cfg.pty_pin,&pin)<0||fstat(fd,&actual)<0)error=errno;
        else if(!S_ISCHR(actual.st_mode)||pin.st_dev!=actual.st_dev||pin.st_ino!=actual.st_ino)error=ESTALE;
        else{
          struct seccomp_notif_addfd add={.id=n->id,.flags=SECCOMP_ADDFD_FLAG_SEND,
            .srcfd=(unsigned)fd,.newfd_flags=(flags&O_CLOEXEC)?O_CLOEXEC:0};
          int inserted=ioctl(listener,SECCOMP_IOCTL_NOTIF_ADDFD,&add);int saved=errno;
          close(fd);close(mem);
          if(inserted>=0){grants++;return 0;}
          if(saved==ENOENT)return 0;
          denials++;return reply(listener,n->id,0,saved);
        }
        close(fd);
      }
    }
  }else if(kind==1){
    struct stat st;if(fstat(cfg.pty_pin,&st)<0)error=errno;
    else if(output>INT64_MAX||pwrite(mem,&st,sizeof(st),(off_t)output)!=sizeof(st))error=EFAULT;
    else result=0;
  }else if(kind==4){
    struct statx st;if(syscall(SYS_statx,cfg.pty_pin,"",AT_EMPTY_PATH|(flags&AT_STATX_SYNC_TYPE),
        (unsigned)n->data.args[3],&st)<0)error=errno;
    else if(output>INT64_MAX||pwrite(mem,&st,sizeof(st),(off_t)output)!=sizeof(st))error=EFAULT;
    else result=0;
  }else error=EACCES;
...
static int acquisition(int listener,const struct seccomp_notif *n) {
  ...
  /* The terminal alias is copied from stopped task memory and resolves only
   * to this supervisor's pinned private slave. No global /dev grant exists. */
  if(!strcmp(path,"/dev/tty"))return own_terminal(listener,n,mem,kind,flags,output);
...
static int private_terminal(void) {
  cfg.pty_master=posix_openpt(O_RDWR|O_NOCTTY|O_CLOEXEC);
  if(cfg.pty_master<0||grantpt(cfg.pty_master)<0||unlockpt(cfg.pty_master)<0)return -1;
  /* TIOCGPTPEER opens the slave of the held master atomically, with no
   * pathname lookup or reusable pts number between identification and open. */
  int slave=ioctl(cfg.pty_master,TIOCGPTPEER,O_RDWR|O_NOCTTY|O_CLOEXEC);
  if(slave<0)return -1;
  char proc[80];snprintf(proc,sizeof(proc),"/proc/self/fd/%d",slave);
  cfg.pty_pin=open(proc,O_PATH|O_CLOEXEC);
  struct stat st;struct winsize size={.ws_row=cfg.rows,.ws_col=cfg.columns},actual={0};
  if(cfg.pty_pin<0||fstat(cfg.pty_pin,&st)<0||!S_ISCHR(st.st_mode)||
     ioctl(cfg.pty_master,TIOCSWINSZ,&size)<0||ioctl(slave,TIOCGWINSZ,&actual)<0||
     actual.ws_row!=cfg.rows||actual.ws_col!=cfg.columns||nonblock(cfg.pty_master)<0){
    int saved=errno;close(slave);errno=saved?saved:EIO;return -1;}
  return slave;
}
static int kill_owned_children(void) {
  /* Numeric /proc entries are only candidates. A pidfd pins each identity;
   * waitid(P_PIDFD) verifies our child relationship before any signal. */
  char path[96];snprintf(path,sizeof(path),"/proc/self/task/%d/children",getpid());
  FILE *children=fopen(path,"re");if(!children)return -1;
  int error=0;unsigned long number;
  while(fscanf(children,"%lu",&number)==1){
    if(!number||number>INT_MAX){error=EPROTO;break;}
    int fd=(int)syscall(SYS_pidfd_open,(pid_t)number,0);
    if(fd<0){if(errno==ESRCH)continue;error=errno;break;}
    siginfo_t info={0};
    if(waitid(P_PIDFD,(id_t)fd,&info,WEXITED|WNOHANG|WNOWAIT)<0){
      int saved=errno;close(fd);if(saved==ECHILD||saved==ESRCH)continue;error=saved;break;}
    if(syscall(SYS_pidfd_send_signal,fd,SIGKILL,NULL,0)<0&&errno!=ESRCH)error=errno;
    close(fd);if(error)break;
  }
  if(!error&&(ferror(children)||!feof(children)))error=EPROTO;
  fclose(children);if(error){errno=error;return -1;}return 0;
}
static int supervise(void) {
  int setup[2],transfer[2],slave;
  if(pipe2(setup,O_CLOEXEC)<0||socketpair(AF_UNIX,SOCK_SEQPACKET|SOCK_CLOEXEC,0,transfer)<0||
     prctl(PR_SET_CHILD_SUBREAPER,1,0,0,0)<0||(slave=private_terminal())<0)return no_worker_result(20,errno);
  rlim_t tasks;if(uid_tasks(&tasks)<0)return no_worker_result(21,errno);
  pid_t parent=getpid();leader=fork();if(!leader)child(slave,setup[1],transfer[1],parent,tasks);
  int fork_error=errno;close(slave);close(setup[1]);close(transfer[1]);if(leader<0)return no_worker_result(22,fork_error);
  int listener=-1,setup_ok=0,setup_eof=0,terminal_eof=0,input_eof=0,observed=0,empty=0,started=0,status=0;
  int stage=0,saved_error=0,terminating=0,held=0,output_broken=0;int64_t cleanup=0;
  const char *outcome="exited";uint64_t bytes=0;unsigned char setup_data[16],input[65536],output[65536];
  size_t setup_size=0,input_size=0,output_size=0;
  if(nonblock(setup[0])<0||nonblock(transfer[0])<0||nonblock(cfg.input)<0){cancelled=1;outcome="setup_error";}
  struct seccomp_notif_sizes sizes={0};if(syscall(SYS_seccomp,SECCOMP_GET_NOTIF_SIZES,0,&sizes)<0){cancelled=1;outcome="setup_error";}
  struct seccomp_notif *notification=calloc(1,sizes.seccomp_notif?sizes.seccomp_notif:sizeof(struct seccomp_notif));
  if(!notification){cancelled=1;outcome="setup_error";}
  while(!empty||!terminal_eof||!setup_eof||output_size){
    int64_t now=milliseconds();if(commands()<0){cancelled=1;outcome="broker_error";}
    if(!terminating&&(cancelled||now>=deadline)){terminating=1;cleanup=now+5000;
      if(!strcmp(outcome,"exited"))outcome=cancelled?"cancelled":"timeout";}
    if(terminating){input_size=0;input_eof=1;if(kill_owned_children()<0){saved_error=errno;outcome="cleanup_error";}}
    if(terminating&&now>=cleanup&&!held){held=1;outcome="cleanup_error";
      const char *event="{\"type\":\"ownership-retained\",\"cleanupComplete\":false}";(void)packet(event,strlen(event),1);}
    struct pollfd p[7]={
      {terminal_eof?-1:cfg.pty_master,(short)((output_size<sizeof(output)?POLLIN:0)|(input_size?POLLOUT:0)),0},
      {input_eof||input_size==sizeof(input)?-1:cfg.input,POLLIN,0},
      {setup_eof?-1:setup[0],POLLIN,0},{listener<0?transfer[0]:-1,POLLIN,0},
      {terminating?-1:listener,POLLIN,0},{cfg.command,POLLIN,0},{output_size?1:-1,POLLOUT,0}};
    if(poll(p,7,held?1000:20)<0&&errno!=EINTR){cancelled=1;outcome="broker_error";}
    if(listener<0&&(p[3].revents&POLLIN)){listener=receive_fd(transfer[0]);
      if(listener<0||nonblock(listener)<0){cancelled=1;outcome="setup_error";}}
    if(listener>=0&&(p[4].revents&POLLIN)&&!terminating&&notification){memset(notification,0,sizes.seccomp_notif);
      if(ioctl(listener,SECCOMP_IOCTL_NOTIF_RECV,notification)<0){
        if(errno!=EINTR&&errno!=ENOENT&&errno!=EAGAIN){cancelled=1;outcome="broker_error";}}
    ...
    if(p[1].revents&(POLLIN|POLLHUP|POLLERR)){
      ssize_t n=read(cfg.input,input+input_size,sizeof(input)-input_size);
      if(n>0)input_size+=(size_t)n;
      else if(!n)input_eof=1; /* PTY input has no synthetic pipe half-close. */
      else if(errno!=EINTR&&errno!=EAGAIN){cancelled=1;outcome="broker_error";}}
    if(input_size&&(p[0].revents&POLLOUT)){
      ssize_t n=write(cfg.pty_master,input,input_size);
      if(n>0){input_size-=(size_t)n;memmove(input,input+n,input_size);}
      else if(n<0&&errno!=EINTR&&errno!=EAGAIN){input_size=0;input_eof=1;
        if(errno!=EIO){cancelled=1;outcome="broker_error";}}}
    if(output_size<sizeof(output)&&(p[0].revents&(POLLIN|POLLHUP|POLLERR))){
      ssize_t n=read(cfg.pty_master,output+output_size,sizeof(output)-output_size);
      if(!n||(n<0&&errno==EIO))terminal_eof=1;
      else if(n>0){size_t amount=(size_t)n;
        if(cfg.output&&bytes+output_size+amount>cfg.output){amount=(size_t)(cfg.output-bytes-output_size);
          cancelled=1;outcome="output_limit";}
        if(!output_broken)output_size+=amount;}
      else if(errno!=EINTR&&errno!=EAGAIN){cancelled=1;outcome="broker_error";}}
    if(output_size&&(p[6].revents&(POLLOUT|POLLHUP|POLLERR))){
      ssize_t n=write(1,output,output_size);
      if(n>0){bytes+=(uint64_t)n;output_size-=(size_t)n;memmove(output,output+n,output_size);}
      else if(!n||(errno!=EINTR&&errno!=EAGAIN)){output_broken=1;output_size=0;cancelled=1;outcome="broker_error";}}
    if(p[2].revents&(POLLIN|POLLHUP|POLLERR)){
      unsigned char data[16];ssize_t n=read(setup[0],data,sizeof(data));
      if(!n)setup_eof=1;
      else if(n>0){if(setup_size+(size_t)n>sizeof(setup_data)){cancelled=1;outcome="setup_error";}
        else{memcpy(setup_data+setup_size,data,(size_t)n);setup_size+=(size_t)n;}}
      else if(errno!=EINTR&&errno!=EAGAIN){cancelled=1;outcome="setup_error";}}
    if(setup_eof&&!started&&!terminating&&!cancelled&&milliseconds()<deadline){
      ...
      if(!setup_ok||listener<0){cancelled=1;outcome="setup_error";
        if(setup_size>=8){memcpy(&stage,setup_data+setup_size-8,4);memcpy(&saved_error,setup_data+setup_size-4,4);}}
      else{const char *event="{\"type\":\"started\",\"profile\":\"bionic-host-pty-v1\",\"setupCompleted\":true}";
        if(packet(event,strlen(event),0)<0){saved_error=errno;outcome=saved_error==ECANCELED?(cancelled?"cancelled":"timeout"):"broker_error";cancelled=1;}
        else started=1;}}
    if(!observed){siginfo_t info={0};if(waitid(P_PID,(id_t)leader,&info,WEXITED|WNOHANG|WNOWAIT)==0&&info.si_pid==leader){
      observed=1;if(!terminating){terminating=1;cleanup=milliseconds()+5000;}}}
    if(terminating)for(;;){int current;pid_t pid=waitpid(-1,&current,WNOHANG);
      if(pid>0){if(pid==leader){reaped=1;status=current;char event[128];
          int count=snprintf(event,sizeof(event),"{\"type\":\"exited\",\"exitCode\":%d,\"signal\":%d}",
            WIFEXITED(status)?WEXITSTATUS(status):-1,WIFSIGNALED(status)?WTERMSIG(status):0);(void)packet(event,(size_t)count,1);}continue;}
      if(pid<0&&errno==ECHILD)empty=1;
      break;}
  }
  free(notification);if(listener>=0)close(listener);close(transfer[0]);close(setup[0]);close(cfg.pty_pin);close(cfg.pty_master);cfg.pty_master=-1;
  ...
  char final[768];int length=snprintf(final,sizeof(final),"{\"type\":\"result\",\"outcome\":\"%s\",\"exitCode\":%d,\"signal\":%d,\"cleanupComplete\":%s,\"started\":%s,\"grants\":%llu,\"denials\":%llu,\"stdoutBytes\":%llu,\"stderrBytes\":0,\"stage\":%d,\"errno\":%d}",
    outcome,code,sig,reaped&&empty?"true":"false",started?"true":"false",(unsigned long long)grants,(unsigned long long)denials,(unsigned long long)bytes,stage,saved_error);
...
int main(int argc,char **argv) {
  ...
  cfg.root=fds[0];cfg.input=fds[1];cfg.control=fds[2];cfg.command=fds[3];cfg.pty_master=-1;cfg.pty_pin=-1;
  ...
  umask(0077);deadline=cfg.wall?milliseconds()+(int64_t)cfg.wall:INT64_MAX;
  ...
  const char *ready="{\"type\":\"ready\",\"profile\":\"bionic-host-pty-v1\"}";
