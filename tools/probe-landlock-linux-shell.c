...
/* FIXED LINUX SHELL THROUGH PROOT: feasibility only, NOT a production sandbox.
 ...
 * Unknown syscalls fail EPERM. No sockets/network, namespace or mount syscalls
 * are permitted by our filter. PRoot namespace emulation is NOT isolation.
 *
 * Fixed script only, no user command input, no startup/preload file or account
 * access. /etc/ld.so.preload is bound to /dev/null without changing rootfs files.
...
static const char shell_script[] =
    ...
    ".agents/settings ../outside/victim.txt /outside/victim.txt outside-link/victim.txt; do\n"
...
static void child_probe(int channel, int workspace, int base, int outside) {
    ...
    execl(proot_path, proot_path,
          ...
          "PATH=/usr/bin:/bin", "HOME=/foldgpt-fixture", "LC_ALL=C",
          "/bin/sh", "-c", shell_script, "foldgpt-linux-shell-probe", (char *)NULL);
...
static void handle_notification(int listener, int workspace, pid_t child,
    ...
    if (request->data.nr != SYS_openat) { deny_request(listener, id, EPERM); return; }
...
int main(int argc, char **argv) {
...
#ifdef __ANDROID__
    int length = snprintf(evidence, sizeof(evidence), "%s/cache/foldgpt-linux-shell-XXXXXX", parent_path);
...
#else
    int length = snprintf(evidence, sizeof(evidence), "%s/foldgpt-linux-shell-XXXXXX", parent_path);
    ...
    if (!child) {
        close(channels[0]);
        child_probe(channels[1], workspace, base, outside);
        _exit(1);
    }
    close(channels[1]);
    ...
    require(broker_grants == 4 && descendant_workspace_grants == 4 && broker_denials == 10,
            "expected Linux shell broker request counts");
    close(workspace);
    close(outside);
    close(scratch_directory);
    close(base);
    printf("independent_parent_verification=PASS evidence_directory=%s\n", evidence);
    puts("scope=fixed Linux shell via PRoot; not Codex or an arbitrary-command sandbox");
    return 0;
}
