...
The [repository layout](project-layout.md) explains where maintained sources live. The [test guide](../tests/README.md) distinguishes host regressions, native checks and device qualification.
...
Dated verification reports are kept under `history/` as engineering context; release scope records are under `releases/`. Use the [compatibility matrix](compatibility.md) for current public support statements and the [changelog](../CHANGELOG.md) for release scope.
