...
See [r25 evidence](../research/r25-device-validation-20260908.md).
...
| Native managed acquisition | A fixed static ARM64 worker passed 17 tests and 46 actual process observations with the graphical client running. Real Landlock/seccomp and FD injection enforce the admitted file policy; tests cover reads/writes/creation, denials and metadata exceptions, copied-pointer mutation, FD flags, cancellation, timeout and forked descendant cleanup. Independent collection verifies the tested APK, 30 installed libraries, eight executed sources and complete event coverage. This is a static acquisition profile, without normal model work or the official process RPC lifecycle. See [Android evidence](../../tools/executor/native-managed-android.md). |
| Native process lifecycle | The fixed static lifecycle backend passed 20 tests and 104 observations on Android/Bionic. Independent collection binds the executed sources, 32 packaged libraries, APK and native event/stream evidence. The actual libc memfd/seal test fixes the missing CPython API without replacing sealed memory with an ordinary file. This remains direct backend conformance; composed transport and normal model routing are unfinished. See [Android lifecycle evidence](../../tools/executor/native-processes-android.md). |
| HTTPS acquisition | A separately authenticated official 26.901.51231 package was downloaded on the Fold, rehashed and reused from the same private inode under a worker-only network-death guard. Independent collection verifies the actual package hash, cache modes, descriptor, APK and service completion. The client was not installed or activated, so this does not qualify an update. See [acquisition evidence](../../tools/install/https-acquisition/README.md). |
| Android agent context | The exact global AGENTS block was loaded into two real new Desktop tasks on the Fold with 5.6 Luna. The model identified Android, Debian/PRoot, local rendering versus remote inference and the exact resolved manifest, without those answers in the test prompt. The current map does not pin a model. This is local Codex context delivery, not protected command execution or Android API access. See [context evidence](../agent-environment.md). |
| Inactive installation v2 | The Android coordinator prepared authenticated Debian, its guest account, the intact official client, vault and GNOME collection twice, with independent verification of retained identities. The root remains `PREPARED`; no activation, client launch or model task was attempted. See [combined preparation](../install/combined-preparation-probe.md). |
| Inactive installation v3 | Two real Android calls prepared/revalidated integration, client and vault after recovery of a permission conflict. Independent native collection matched all 345 entries: 29 integration and 316 XKB entries, plus retained root/package/report identities and hashes. The stage remains `PREPARED`, without activation. See [v3 evidence](../install/inactive-native-integration.md). |
...
The newer [v3 inactive integration route](../install/inactive-native-integration.md)
...
- The compatibility shim simulates isolation calls. It does not provide equivalent Linux namespace protection; see [NATIVE-AUDIT.md](../history/native-startup-2026-09-05.md).
- Adreno rendering and the reproduced menu defect pass the foldgpt5 checks. Broader reliability, thermal and battery testing remains; see the [current GPU report](../verification-gpu-renderpasses-2026-09-06.md), including the separate unresolved diagnostic-color investigation.
