 ...
 */
static Bool lorieOwnsAddedKey(XkbDescPtr xkb, AddedKeySym *item)
{
    return XkbKeycodeInRange(xkb, item->keycode) &&
           XkbKeyNumGroups(xkb, item->keycode) > 0 &&
           XkbKeySymsPtr(xkb, item->keycode)[0] == item->keysym &&
           (xkb->names == NULL || xkb->names->keys == NULL ||
            xkb->names->keys[item->keycode].name[0] == 'T');
}

static KeyCode getReusableKeycode(XkbDescPtr xkb, XkbDescPtr source)
{
    struct xorg_list *entry, *previous;
    DeviceIntPtr master = GetMaster(lorieKeyboard, KEYBOARD_OR_FLOAT);

    for (entry = addedKeysyms.prev; entry != &addedKeysyms; entry = previous) {
        AddedKeySym *last = xorg_list_entry(entry, AddedKeySym, entry);
        KeyCode key = last->keycode;
        previous = entry->prev;

        /* A down key must keep its mapping until its matching release. */
        if (pressedKeys[key] != NoSymbol ||
            key_is_down(master, key, KEY_PROCESSED) ||
            key_is_down(lorieKeyboard, key, KEY_PROCESSED))
            continue;

        /* A client can independently replace either device's mapping. */
        Bool owned = lorieOwnsAddedKey(xkb, last) &&
                     (source == xkb || lorieOwnsAddedKey(source, last));
...
static Bool lorieSetKeySymbols(XkbDescPtr xkb, KeyCode key, KeySym lower,
                              KeySym upper, XkbChangesPtr changes)
{
    int types[1] = { lower == upper ? XkbOneLevelIndex : XkbAlphabeticIndex };
    KeySym *syms;

    memset(changes, 0, sizeof(*changes));
    if (XkbChangeTypesOfKey(xkb, key, 1, XkbGroup1Mask, types,
                           &changes->map) != Success)
        return FALSE;
...
static void lorieNotifyNewKeymap(DeviceIntPtr device)
{
    XkbDescPtr xkb = device->key->xkbInfo->desc;
    xkbNewKeyboardNotify notification;

    memset(&notification, 0, sizeof(notification));
    notification.deviceID = notification.oldDeviceID = device->id;
    notification.minKeyCode = notification.oldMinKeyCode = xkb->min_key_code;
    notification.maxKeyCode = notification.oldMaxKeyCode = xkb->max_key_code;
    notification.changed = XkbNKN_KeycodesMask;
    /* This change originates in the virtual keyboard, not an X client request. */
    XkbSendNewKeyboardNotify(device, &notification);
}

static KeyCode lorieAddKeysym(KeySym keysym, unused unsigned state) {
    DeviceIntPtr master = GetMaster(lorieKeyboard, KEYBOARD_OR_FLOAT);
    XkbDescPtr xkb = master->key->xkbInfo->desc;
    XkbDescPtr source = lorieKeyboard->key->xkbInfo->desc;
    XkbEventCauseRec cause;
    XkbChangesRec changes, sourceChanges;
    KeySym upper, lower;
    unsigned int key;

    for (key = xkb->max_key_code; key >= xkb->min_key_code; key--) {
        if (XkbKeycodeInRange(source, key) &&
            XkbKeyNumGroups(xkb, key) == 0 &&
            XkbKeyNumGroups(source, key) == 0)
            break;
    }
    if (key < xkb->min_key_code)
        key = getReusableKeycode(xkb, source);
    if (!key)
        return 0;
    ...
    /*
     * The input event originates on the Lorie slave, while core clients read
     * the master map and XI clients can read the source map. Keep both maps
     * consistent, as ProcXkbSetMap does for a master and its attached slaves.
     * Updating only the master leaves XI clients with NoSymbol and allows a
     * later CopyKeyClass (slave switch) to discard the dynamically added keys.
     * Mutate both descriptions before publishing either notification.
     */
    if (source != xkb &&
        !lorieSetKeySymbols(source, key, lower, upper, &sourceChanges))
        return 0;
    if (!lorieSetKeySymbols(xkb, key, lower, upper, &changes)) {
        /* Publish any successful source update even if master allocation failed. */
        if (source != xkb)
            XkbSendNotification(lorieKeyboard, &sourceChanges, &cause);
        return 0;
    }

    saveAddedKeysym(key, lower);
    if (source != xkb)
        XkbSendNotification(lorieKeyboard, &sourceChanges, &cause);
    XkbSendNotification(master, &changes, &cause);

    /*
     * Adding/reassigning a named logical key replaces the virtual keyboard's
     * layout. Publish that description change as XkbDeviceApplyKeymap does,
     * even though the device ID and legal keycode range remain unchanged.
     * Chromium selects NewKeyboardNotify but not MapNotify; XKB consequently
     * suppresses its core MappingNotify events. Without this notification its
     * key cache remains stale, independently of the input-method connection.
     * Keep the fine-grained map/name notifications for clients using those.
     * Existing key mappings never take this path, and both device descriptions
     * are current before the first notification and before any key press.
     */
    if (source != xkb)
        lorieNotifyNewKeymap(lorieKeyboard);
    lorieNotifyNewKeymap(master);
 ...
 *
 * Server thread only: mapping changes, client notifications and input dispatch
 * must be serialized with XKB requests. The Lorie socket uses SetNotifyFd for
 * this reason; input_lock alone does not protect the client's map or output.
 */
void lorieKeysymKeyboardEvent(KeySym keysym, int down) {
    int i;
    ...
    if (!down) {
         ...
         * key presses.
         */
        LogMessageVerb(X_DEBUG, -1, "Unexpected release of an unpressed symbol\n");
        return;
    }
    ...
    if (keycode == 0) {
        ...
        if (keycode == 0) {
            LogMessageVerb(X_ERROR, -1, "Failure adding a keyboard symbol\n");
            return;
        }
        ...
        keycode = lorieKeysymToKeycode(keysym, state, &new_state);
        if (keycode == 0) {
            LogMessageVerb(X_ERROR, -1, "Newly added keyboard symbol cannot be generated\n");
            return;
        }
    }
    ...
    if (((state & ShiftMask) != (new_state & ShiftMask)) && lorieIsAffectedByNumLock(keycode)) {
        ...
        LogMessageVerb(X_DEBUG, 0, "Finding alternative symbol to avoid fake shift for numpad\n");
    ...
    for (i = 0;i < 256;i++) {
        if (i == keycode)
            continue;
        if (pressedKeys[i] == keysym) {
            LogMessageVerb(X_ERROR, -1, "Keyboard symbol generated by multiple keys\n");
            pressedKeys[i] = NoSymbol;
        }
    }
