...
void Renderer::notifyGpuCopyDone() const {
    if (clientWriter) {
        ...
        clientWriter->sendFrame(&e, sizeof(e));
    }
}
...
void Renderer::applyPendingGpuCopies() {
    ...
    // Socket writers may wait for one another. Release the shared X-server
    // lock first so its reader can finish dispatching and drain the stream.
    if (serial)
        notifyGpuCopyDone();
}
