...
//        mEdgeSlopInPx = ViewConfiguration.get(/*desktop*/ ctx).getScaledEdgeSlop();
    ...
    public boolean handleTouchEvent(View view0, View view, MotionEvent event) {
        ...
        // AccessibilityService.dispatchGesture supplies touchscreen events whose
        // tool type may be UNKNOWN. They still carry ordinary absolute touch
        // coordinates; dropping them makes authorized Android gestures complete
        // successfully while never reaching the Linux window.
        int touchTool = event.getToolType(event.getActionIndex());
        if (touchTool == MotionEvent.TOOL_TYPE_FINGER
                || (touchTool == MotionEvent.TOOL_TYPE_UNKNOWN
