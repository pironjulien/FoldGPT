...
- Complete protected local Codex model execution has not passed. The private file bridge and static managed-acquisition suite are verified components; the official process lifecycle, production Desktop routing, general shell/runtime admission, full filesystem compatibility, stdin/TTY and networking still need integration and validation. The existing path's Debian `bwrap` failure remains a recorded blocker.
